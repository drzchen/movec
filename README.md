...
The SMATUS approach has been implemented in a stand-alone tool, named Movec, which transforms a C source file into an instrumented C source file that can be compiled by a standard C compiler such as gcc.
...
Movec is developed and extensively tested on Ubuntu 16.04 LTS 64-bit. It may also work on other compatible Linux releases, but it is not guaranteed to do so.

Compiling the programs in the benchmark suites and evaluating tools on them require that you have several software packages installed. For example, make, gcc, and the usual plethora of Unix utilities such as cp, find, mkdir, mv, rm, time etc.

Movec depends on a JSON compilation database when instruments a project containing multiple source files. Thus you should also install Bear, a tool that generates a JSON compilation database for the make command. You can install Bear via the Ubuntu software repository.
```Bash
...
```Bash
  ...
  $ ./all
```
The second command above generates a compilation database named "compile_commands.json" in the directory containg the Makefile. Do not run "bear make" again, otherwise the JSON file is empty. Note that this is the most convenient way when the compilation process is complex, e.g., when there are too many files to be manually listed and compiling each source file uses different compiler options.
...
* Go to the parent directory containing llvm, or where you want to build llvm.
...
```
Running a serial build will be slow. You may run a parallel build, for make, use make -j. The last command installs LLVM header files, libraries, tools, and documentation in a hierarchy under $PREFIX, specified with CMAKE_INSTALL_PREFIX, which defaults to /usr/local. If you do not want to install, add llvm-build/bin to your PATH.

## 1.4 SoCets Setup

You can download its source package from [SoftboundCets' page](https://www.cs.rutgers.edu/~santosh.nagarakatte/softbound/).
...
Valgrind can be obtained from any GNU Unix repositories via apt-get or yum. For example, on Debian/Ubuntu-like distributions, do
```Bash
    $ sudo apt-get install valgrind
```

If you already have its Debian package, then you can install Valgrind from Debian package by following the following instructions.
```Bash
...
The directory of each benchmark suite (e.g., SARD+memsafe, MiBench, SPEC) contains several files. Please read the following files:

* Makefile.default.TOOL (or Makefile.default for SARD+memsafe): It contains the script for running the TOOL on a benchmark program. Note that you should correctly set the path of the TOOL's executable file at the beginning lines of this file (after comments).
...
```Bash
    $ cd benchmarks-xxx [xxx may be memsafe, mibench or spec-cpu-2017]
    $ make build
    $ make run   [for SARD+memsafe]
    $ make large [for MiBench]
    $ make test  [for SPEC]
...
```Bash
    ...
    $ make BENCHMARK/run   [for SARD+memsafe]
...
```Bash
    ...
    $ make run-movec-memsafe O=-O3   [for SARD+memsafe]
...
```Bash
    ...
    $ make BENCHMARK/run-movec-memsafe   [for SARD+memsafe]
...
```Bash
    ...
    $ make BENCHMARK/run-movec-memsafe O=-O3   [for SARD+memsafe]
...
```Bash
    ...
    $ make run-addresssanitizer   [for SARD+memsafe]
