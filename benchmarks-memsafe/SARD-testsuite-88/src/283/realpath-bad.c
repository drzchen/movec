...
/* #ifdef HAVE_LSTAT */
    ...
    printf ("resolved(%lu) = %s\n", strlen(resolved), resolved);
    printf ("wbuf(%lu) = %s \n", strlen(wbuf), wbuf);
    printf ("rootd=%d\n", rootd);    
    printf("strlen(resolved) + strlen(wbuf) + rootd + 1 = %lu\n", strlen(resolved) + strlen(wbuf) + rootd + 1);
